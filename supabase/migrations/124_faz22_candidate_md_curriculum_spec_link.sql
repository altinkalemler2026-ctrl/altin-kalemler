-- 124_faz22_candidate_md_curriculum_spec_link.sql
-- Altın Kalemler
--
-- Faz 22 kapanış düzeltmesi: her geçerli candidate-md staging yazımında
-- Faz 118 ile aynı atomik zinciri kur.
--
-- Önceki
--   123 (v1.1 markdown intake) her geçerli adayı staging'e yazıyordu ancak
--   ai_generation_specs kaydı oluşturmuyor ve staging'i spec'e bağlamıyordu.
--   Bu yüzden 034 start_curriculum_fit_verification → "Generation spec is
--   required" ve bağımsız doğrulama hattı (start_answer_verification dahil)
--   spec okuması yapamıyordu.
--
-- Çözüm (append-only; 123 değiştirilmez):
--   1. private.register_candidate_md_batch AYNI imzayla (jsonb → jsonb)
--      CREATE OR REPLACE edilir; 123'ün tüm doğrulama/idempotency/audit/
--      yayın-yasağı/preflight davranışı birebir korunur.
--   2. Her geçerli aday için, ders (subject) çözümünden SONRA outcome_code
--      → curriculum_outcomes (aktif, varsayılan versiyon öncelikli) atomik
--      çözümü eklenir ve FAIL-CLOSED yapılır: çözülemezse aday yeni hata kodu
--      'outcome_not_found' ile reddedilir; spec/staging/validation YAZILMAZ,
--      yalnız audit'te 'invalid' kalır (yarım ilişki bırakmaz).
--   3. Geçerli aday ilk kez staging'e yazılırken önce 118 sözleşmesine uygun
--      ai_generation_specs kaydı oluşturulur (desired_count=1, status='ready',
--      grade/subject/outcome, difficulty/cognitive/solve-time, provenance
--      constraints) ve staging satırı generation_spec_id + proposed_*
--      kolonlarıyla bağlanır; metadata'ya required_outcome_id /
--      generation_spec_id / curriculum_version_id eklenir (034 gate'inin
--      beklediği snapshot).
--
-- Idempotent: CREATE OR REPLACE + REVOKE/GRANT; tekrar uygulama hata vermez.

BEGIN;


-- =========================================================
-- 1. İNTAKE RPC (PRIVATE) — 123 davranışı + Faz 118 zinciri
-- =========================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Aktif kazanım tek bir id'ye çözülmeli; çözülemezse aday
    -- 'outcome_not_found' ile reddedilir ve yarım spec/staging/
    -- validation ilişkisi bırakılmaz.
    -- -----------------------------------------------------
    ...
    -- desired_count=1, status='ready', outcome_id ve provenance
    -- constraints; yalnız geçerli + inserted adaylar için.
    -- -----------------------------------------------------
...
-- =========================================================
-- 2. PUBLIC sarmalayıcı (123'te tanımlı; 118 deseniyle aynı
--    SECURITY INVOKER facade) bu migration'dan DEĞİŞMEZ.
-- =========================================================
